...
class PointDetector(nn.modules.Module):
    ...
    def post_processing(self, data_dict):
        ...
        points_pred_batch = []
        slots_pred = []
        for b, marks in enumerate(points_pred):  # (3,16,16)
            points_pred = get_predicted_points(marks, self.cfg.point_thresh, self.cfg.boundary_thresh)  # 0.008 0.05 得到一个list，当前世4个元素
